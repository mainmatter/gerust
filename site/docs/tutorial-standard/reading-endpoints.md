...
All of these tests use Gerust's `#[db_test]` macro (defined in the [`macros` crate](/architecture/the-macros-crate)) instead of Rust's (or tokio's) `#[test]` macro. The `#[db_test]` macro will cause the test function to be invoked with a `context: &DbTestContext` argument that allows access to a database pool as well as the application under test. Gerust creates a fresh database for each test case that's specific for the test (and created using the main test database as a template). The application instance being tested (and made available via the `DbTestContext`'s `app` property) is configured to use that database and the test itself has access to the database as well via the `DbTestContext`'s `db_pool` property. That database is automatically deleted once the test completes so that no unused databases are left behind.

The test functions for the `notes` controller populate the test database with fake data, e.g.:
...
They can then call the respective endpoint of the application using the helper functions defined on `context.app`, e.g.:
